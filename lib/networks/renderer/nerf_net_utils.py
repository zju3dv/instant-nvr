...
def raw2outputs_pixelnerf(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False):
    """Transforms model's predictions to semantically meaningful values.
    Args:
        raw: [num_rays, num_samples along ray, 4]. Prediction from model.
        z_vals: [num_rays, num_samples along ray]. Integration time.
    ...
    # obtain the biggest occupancy value along each camera ray
    occ_map = torch.max(alpha, dim=1)[0]

    return rgb_map, disp_map, acc_map, weights, depth_map, occ_map
