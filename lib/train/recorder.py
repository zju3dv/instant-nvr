...
def process_inb(img_stats):
    for k, vs in img_stats.items():
        for i, v in enumerate(vs):
            if len(v.shape) == 2:
                v = v[..., None]
            vs[i] = v
        img_stats[k] = torch.stack(vs)
    return img_stats
...
class Recorder(object):
    ...
    def record(self, prefix, step=-1, loss_stats=None, image_stats=None):
        ...
        for k, v in image_stats.items():
            self.writer.add_images(pattern.format(k), v, step, dataformats='NHWC')
    ...
    def __str__(self):
        ...
        recording_state = '  '.join(['epoch: {}', 'step: {}', '{}'])
        return recording_state.format(self.epoch, self.step, loss_state)
